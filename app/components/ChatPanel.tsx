...
export function ChatPanel() {
  ...
  return (
    <div className='flex flex-col h-full'>
      ...
      <form onSubmit={handleSendMessage} className='absolute bottom-0 left-0 right-0 p-4 bg-black/10 backdrop-blur-sm'>
        <div className='flex max-w-[1200px] mx-auto'>
          <div className='flex w-full rounded-lg overflow-hidden border border-yellow-500/20'>
            <input
              type='text'
              value={inputMessage}
              onChange={(e) => setInputMessage(e.target.value)}
              placeholder='Type your message...'
              className='flex-1 px-4 py-3 bg-black/50 text-white placeholder:text-gray-400 focus:outline-none text-sm'
              onKeyDown={(e) => {
